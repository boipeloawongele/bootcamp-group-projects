...
- These questions are typical **project reflection/oral assessment** questions. Here are strong, concise answers you can use or adapt.

### 1. Does the `parse_state` function return enough information to generate legal actions and apply one?
...
### 2. Is it easy to check whether an action is currently legal given the decision point?
...
### 3. Can every member of the team explain why Double Down, Split, Surrender, and Insurance all share the same "first decision only" restriction?
...
### 4. Does the `apply_action` function cleanly handle the fact that Split returns two hands while every other action returns one?
...
### 5. What was your approach to teamwork?
...
These improvements would make the project closer to a complete Blackjack game while keeping the current code structure.
